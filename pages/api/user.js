  ...
  export default async function User(req, res) {
    ...
    if (req.method == 'POST') {
        try {
            ...
            const token = await fetch('https://dev-bk5y913p.us.auth0.com/api/v2/oauth/token', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    audience: 'https://dev-bk5y913p.us.auth0.com/api/v2/'
                })
            }).then(res => res.json());

            // Get current user's sub id and update user profile in Auth0 with new edited values
            const { user } = useUser(req, res);
            const updatedUser = await fetch(`https://dev-bk5y913p.us.auth0.com/api/v2/users/${user.sub}`, {
                method: 'PATCH',
                headers: {
